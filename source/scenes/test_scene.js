...
export class TestScene extends PhysicsSim {
    ...
    shoot(controls) {
        ...
        if (controls.cannon) {
            ...
            let go = new GameObject(name, this.shapes.sphere, this.materials.plastic.override({ color: color(1, 0, 1, 1) }), controls.cannon_position, vec3(0, 0, 0), vec3(radius, radius, radius), game_object_types.Bullet);
            go.add_rigidbody_component();
            go.get_rigidbody_component().add_force("Shoot", controls.gun_aim.times(speed), false, 1); // Shoot it.
            go.get_rigidbody_component().add_force("Gravity", vec4(0, gravity, 0, 0), true); // Apply gravity.
            go.add_collider_component(collider_types.Sphere, radius);
        ...
        if (controls.low_grav) {
            ...
            let go = new GameObject(name, this.shapes.sphere, this.materials.plastic.override({ color: color(0, 1, 1, 1) }), controls.low_grav_position, vec3(0, 0, 0), vec3(radius, radius, radius), game_object_types.Bullet);
            go.add_rigidbody_component();
            go.get_rigidbody_component().add_force("Shoot", controls.gun_aim.times(speed), false, 1); // Shoot it.
            go.get_rigidbody_component().add_force("Gravity", vec4(0, gravity, 0, 0), true); // Apply gravity.
            go.add_collider_component(collider_types.Sphere, radius);
